...
namespace SmartWorkStationDR11A
{
    //机器人
    public abstract class RobotBase
    {
        ...
        */
        ...
        public short RunAction(int Action)
        {
            ...
            for (Cntr = 0; Cntr < COM_LEN; Cntr++)
            {
                if (m_SendBuf[Cntr] != InBuf[Cntr]) break;
            }
            if (Cntr >= COM_LEN)
            {
                for (Cntr = 0; Cntr < TIMEOUT; Cntr++)
                {
                    if ((InBuf[6] == 0x00) && (InBuf[9] == 0x0d) && (InBuf[12] == 0x00))
                    {
                        break;
                    }
                    else if ((InBuf[6] == 0xff) && (InBuf[9] == 0x0d) && (InBuf[12] == 0x00))
                    {
                        return -2;  // CRC error
                    }
                    else
                ...
                if (Cntr >= TIMEOUT)
                {
                    return -1;  // It's normal to stop in 3 seconds.
                }
                else
                {
                    return 0;
            ...
            else
            {
                ...
                if (Cntr < COM_LEN)
                {
                    return -1;  // communication shutoff
                }
                else
                {
                    for (Cntr = 0; Cntr < TIMEOUT; Cntr++)
